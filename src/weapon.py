...
class Bullet(Weapon):
    def __init__(self, x: int, y: int, is_enemy: bool = False, velocity_x: int = cfg.bullet_velocity, velocity_y: int = 0):
...
class Bomb(Weapon):
    def __init__(self, x: int, y: int, is_enemy: bool = False, velocity_x: int = 5, velocity_y: int = 0, drag_multiplier: float = 0.1, explosion_power: int = 0):
        ...
        super().__init__(x, y, sprite, is_enemy, explosion_power, velocity_x, velocity_y + cfg.bomb_y_velocity_gain)
